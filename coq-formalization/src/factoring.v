...
Lemma s2r_recl u an' an:
  numq (s2r (rcons (rcons u an') an)) =
    (an.+1%:Z * numq (s2r (rcons u an')) + numq (s2r u))%R /\
  denq (s2r (rcons (rcons u an') an)) =
    (an.+1%:Z * denq (s2r (rcons u an')) + denq (s2r u))%R.
Proof.
  rewrite /s2r !size_rcons -!cats1.
Admitted.
...
Lemma r2s1K x:
  I01 x -> s2r (r2s1 x) = x.
Proof. move=>[H0 H1]. Admitted.

Lemma r2s1_inj x y:
  I01 x -> I01 y -> r2s1 x = r2s1 y -> x = y.
Proof. move=>[H0x H1x] [H0y H1y] Hseq. Admitted.
...
Lemma s2r_inj_l u v:
  s2r u = s2r v -> cast_l u = cast_l v.
Proof. Admitted.

Lemma s2r_inj_s u v:
  s2r u = s2r v -> cast_s u = cast_s v.
Proof. Admitted.
...
Lemma r2s_oddK x:
  I01 x -> s2r (r2s_odd x) = x.
Proof.
  by move => [H0 H1];
  rewrite /r2s_odd;
  case H: (odd (size (r2s1 x)));
  rewrite (r2s1K, r2sK).
Qed.

Lemma r2s_evenK x:
  I01 x -> s2r (r2s_even x) = x.
Proof. 
  by move => [H0 H1];
  rewrite /r2s_even;
  case H: (odd (size (r2s1 x)));
...
Lemma neg_close_conv
  (Iy: I01 y)
  (Hxy: (x < y)%R)
  (Hc: (y - x <= (2 * (denq y)%:Q ^+2)^-1)%R) :
    s2r_ (size (r2s_odd y)) (r2s1 x) = y.
Proof.
  pose d := ((y - x) * (2 * (denq y)%:Q ^+2))%R.
  have Dgt0: (d > 0)%R. admit.
  have Dle1: (d <= 1)%R. admit.
  pose n := size (r2s_odd y).
  (* pose l := (2 * (q_ n * p_ (n-1) - p_ n * q_ (n-1)) / d - q_(n-1) / q_n)%R.  *)
  pose l := ((x - y_ (n-1)) * (x - y))%R.
  have Llt1: (l < 1)%R. admit.
  have Lge0: (l >= 0)%R. admit.
  have Hsx: s2r ((r2s_odd y) ++ (r2s1 l)) = x.
    admit.
  have ->: r2s1 x = (r2s_odd y) ++ (r2s1 l). admit.
  by rewrite s2r_prefix (r2s_oddK Iy).
Admitted.

Lemma close_conv :
  I01 y ->
  (`|x - y| <= (2 * (denq y)%:Q ^+2)^-1)%R ->
    exists t, s2r_ t (r2s1 x) = y.
Proof.
  case: (ltrgtP x y) => Hxy Hy Hc;
  last by (exists (size (r2s1 y));
    rewrite Hxy -{3}(r2s1K Hy)).
  exists (size (r2s_odd y)); exact: neg_close_conv.
Admitted.
...
Definition CF2r c := ((a0 c)%:Q + s2r (s c))%R.

Lemma r2cf_subproof x:
  all (leq 1) (r2s x).
Admitted.

Definition r2cf x :=
  ContF ((numq x)%/(denq x))%Z (r2cf_subproof x).
