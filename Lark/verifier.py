...
class simplifyeq(Transformer):
    ...
    def eq(self, l, r):
        ...
        if(Token('NUMBER','0') in left_terms and len(left_terms) > 1):
            left_terms.remove(Token('NUMBER','0'))
        if(Token('NUMBER','0') in right_terms and len(right_terms) > 1):
            right_terms.remove(Token('NUMBER','0'))
        ...
        ltree = self.build_tree(lterms)
        rtree = self.build_tree(rterms)
    ...
    def build_tree(self, terms):
        if not terms:
            ...
            return Tree('add', terms)
        else:
            return Tree('add', [terms[0],self.build_tree(terms[1:])])
        
## SMT encoding
## Here we provide two alternatives of the encoding for different interpretation of addition
# def tree_to_z3_bool(tree, variables, constraints, bit_width):
#     if isinstance(tree, Token) and tree.type == 'NUMBER':
#         return BitVecVal(tree.value, bit_width)
#     elif tree.data == 'and':
#         return And(*[tree_to_z3_bool(child, variables, constraints, bit_width) for child in tree.children])
#     elif tree.data == 'or':
#         return Or(*[tree_to_z3_bool(child, variables, constraints, bit_width) for child in tree.children])
#     elif tree.data == 'eq':
#         return tree_to_z3_bool(tree.children[0], variables, constraints, bit_width) == tree_to_z3_bool(tree.children[1], variables, constraints, bit_width)
#     elif tree.data == 'leq':
#         return ULE(tree_to_z3_bool(tree.children[0], variables, constraints, bit_width),  tree_to_z3_bool(tree.children[1], variables, constraints, bit_width))
#     elif tree.data == 'geq':   
#         return UGE(tree_to_z3_bool(tree.children[0], variables, constraints, bit_width), tree_to_z3_bool(tree.children[1], variables, constraints, bit_width))
#     elif tree.data == 'add':
#         x = tree_to_z3_bool(tree.children[0], variables, constraints, bit_width)
#         for child in tree.children[1:]:
#             x = x ^ tree_to_z3_bool(child, variables, constraints, bit_width)
#         return x
...
#         return tree_to_z3(tree.children[0], variables, constraints, bit_width) | tree_to_z3(tree.children[1], variables, constraints, bit_width)
#     elif tree.data == 'neg':
#         return Not(tree_to_z3(tree.children[0], variables, constraints, bit_width))
#     elif tree.data == 'var':
...
# ## Deal with the integer addition by using zero extension 
# def tree_to_z3_int(tree, variables, bit_width):

#     if isinstance(tree, Token) and tree.type == 'NUMBER':
#         return BitVecVal(tree.value, bit_width)
#     elif tree.data == 'and':
#         return And(*[tree_to_z3_int(child, variables, bit_width) for child in tree.children])
#     elif tree.data == 'or':
#         return Or(*[tree_to_z3_int(child, variables, bit_width) for child in tree.children])
#     elif tree.data == 'eq':
#         return tree_to_z3_int(tree.children[0], variables, bit_width) == tree_to_z3_int(tree.children[1], variables, bit_width)
#     elif tree.data == 'leq':
#         return ULE(tree_to_z3_int(tree.children[0], variables, bit_width),  tree_to_z3_int(tree.children[1], variables, bit_width))
#     elif tree.data == 'add':
#         temp = tree_to_z3_int(tree.children[0], variables, bit_width)
#         for child in tree.children[1:]:
#             z0 = temp + tree_to_z3_int(child, variables, bit_width)
...
def VCgeneration(precond, program, postcond):
    ...
    cass_expr = simplify(tree_to_z3(cass_tree, {}, 1))
...
# err_cond = """ sum i 1 7 (ex_(i)) <= 1 && sum i 1 25 (ez_(i)) <= 1 """
...
# decoder_cond_int = """ sum i 1 25 (cx_(i)) <= Min( sum i 1 25 (ex_(i)), 2) && sum i 1 25 (cz_(i)) <= Min( sum i 1 25 (ez_(i)), 2) """
...
# #### P_c: error condition
# variables_e = {} 
# err_expr = simplify(tree_to_z3(err_tree.children[0], variables_e,  5))

# #### P_f decoding condition (divided into integer and boolean part to support different interpretation for addition)


# decoder_expr_int = tree_to_z3(decoder_tree_int.children[0],{}, 5)

# variables = {}

# decoder_expr_bool = simplify(tree_to_z3(decoder_tree_bool.children[0], variables, 5))

# #decoding_formula = simplify(Exists([variables_db['s_1'],variables_db['s_2']], And(cass_expr, decoder_expr_int, decoder_expr_bool)))
# decoding_formula1 = simplify(And(cass_expr1, decoder_expr_int, decoder_expr_bool))

# forall_var_list = list(variables.values())
